...
function getFinals(fifaDataArg) {
    const finalsTeams = fifaDataArg.filter(function(item){
...
function getYears(fifaDataArg, getFinalsCB) {
       const years =  getFinalsCB(fifaDataArg).map(item=> item.Year);
       return years;
       console.log(years);
}
...
function getWinners(fifaDataArg, getFinalsCB) {
    return getFinalsCB(fifaDataArg).map(item=> item['Home Team Goals'] > item['Away Team Goals'] ? item['Home Team Name'] : item['Away Team Name']);
}
...
function getWinnersByYear(fifaDataArg, getFinalsCB, getYearsCB, getWinnersCB) {
     const years = getYearsCB(fifaDataArg, getFinalsCB);
     const winners = getWinnersCB(fifaDataArg, getFinalsCB);
